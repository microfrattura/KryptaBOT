...
if (m.message?.protocolMessage?.type === 'MESSAGE_EDIT') {
    ...
    try {
        ...
        let isAdmin = false
        let isRAdmin = false
        let isvortexandvexper = global.owner.some(([num]) => num + '@s.whatsapp.net' === normalizedSender)
        let isROwner = isvortexandvexper || global.owner.some(([num]) => num + '@s.whatsapp.net' === normalizedSender)
        ...
        for (let name in global.plugins) {
            ...
            if (usedPrefix) {
                ...
                if (plugin.vortexandvexper && !isvortexandvexper) {
                    fail('vortexandvexper', m, this)
                    continue
                }
...
global.dfail = async (type, m, conn) => {
    const nome = m.pushName || 'vortexandvexper'
    const etarandom = Math.floor(Math.random() * 21) + 13
    const msg = {
        vortexandvexper: '- 〘 🔒 〙 *`ꪶ͢Comando riservato esclusivamente al creatoreꫂ`*',
