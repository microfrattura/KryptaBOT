...
global.authFile = 'Kryptasessioni';
global.authFileJB = 'Kryptabot-sub';
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
        const violet1 = chalk.hex('#003399');
        const violet2 = chalk.hex('#6495ED');
        const violet3 = chalk.hex('#003399');
        const violet4 = chalk.hex('#6495ED');
        const softText = chalk.hex('#ABCDEF');

        const a = violet1('╭━━━━━━━━━━━━━•💎KryptaBOT💎•━━━━━━━━━━━━━');
        const b = violet1('╰━━━━━━━━━━━━━•💎KryptaBOT💎•━━━━━━━━━━━━━');
        const linea = violet2('   ✦━━━━━━✦✦━━━━━━༺༻━━━━━━༺༻━━━━━━✦✦━━━━━━✦');
...
${chalk.hex('#BB8FCE').italic('\n✧ Suggerimento: Se hai dubbi, scrivi al creatore Vortex o Vexper')}
`);
        }
    ...
    if (opzione === '2' || methodCode) {
        ...
        if (!conn.authState.creds.registered) {
            ...
            if (phoneNumber) {
                ...
                phoneNumber = await question(chalk.bgBlack(chalk.bold.bgblueBright(`Inserisci il numero di WhatsApp.\n${chalk.bold.yellowBright("Esempio: +393123456789")}\n${chalk.bold.blue('━━► ')}`)));
                addNumber = phoneNumber.replace(/\D/g, '');
                if (!phoneNumber.startsWith('+')) phoneNumber = `+${phoneNumber}`;
            }
            setTimeout(async () => {
                let codeBot = await conn.requestPairingCode(addNumber, 'KRYPTA26');
                codeBot = codeBot?.match(/.{1,4}/g)?.join("-") || codeBot;
                console.log(chalk.bold.white(chalk.bgblue('CODICE:')), chalk.bold.white(chalk.white(codeBot)));
            }, 3000);
        }
    }
}
conn.isInit = false;
conn.well = false;
async function byvexperandvortex() {
    try {
...
if (!opts['test']) {
    if (global.db) setInterval(async () => {
        ...
        if (opts['autocleartmp'] && (global.support || {}).find) {
            const tmp = [tmpdir(), 'tmp', "KryptaBOT-sub"];
...
async function connectionUpdate(update) {
    ...
    if (connection === 'open') {
        ...
        if (!global.isLogoPrinted) {
            const finchevedotuttoblublu = [
                '#003399', '#ABCDEF', '#003399', '#ABCDEF', '#003399',
                '#ABCDEF', '#003399', '#ABCDEF', '#003399', '#ABCDEF', '#003399'
            ];
            const KryptaBOT = [`
   ██ ▄█▀ ██▀███  ▓██   ██▓ ██▓███  ▄▄▄█████▓ ▄▄▄       ▄▄▄▄    ▒█████  ▄▄▄█████▓
 ██▄█▒ ▓██ ▒ ██▒ ▒██  ██▒▓██░  ██▒▓  ██▒ ▓▒▒████▄    ▓█████▄ ▒██▒  ██▒▓  ██▒ ▓▒
...
░  ░      ░      ░ ░                        ░  ░ ░          ░ ░             
                 ░ ░               K R Y P T A B O T
            `];
            KryptaBOT.forEach((line, i) => {
                const color = finchevedotuttoblublu[i] || finchevedotuttoblublu[finchevedotuttoblublu.length - 1];
                console.log(chalk.hex(color)(line));
            });
            global.isLogoPrinted = true;
            await byvexperandvortex();
        }
...
async function connectSubBots() {
    const subBotDirectory = './KryptaBOT-sub';
    if (!existsSync(subBotDirectory)) {
        console.log(chalk.bold.blueBright('KryptaBOT non ha Sub-Bot collegati. Creazione directory...'));
        try {
            mkdirSync(subBotDirectory, { recursive: true });
            console.log(chalk.bold.green('✅ Directory KryptaBOT-sub creata con successo.'));
        } catch (err) {
            console.log(chalk.bold.red('❌ Errore nella creazione della directory KryptaBOT-sub:', err.message));
            return;
        }
        return;
    }
    try {
        ...
        if (subBotFolders.length === 0) {
            console.log(chalk.bold.blue('- 🌑 | Nessun subbot collegato'));
            return;
        }
        ...
        if (global.conns.length > 0) {
            console.log(chalk.bold.blueBright(`🌙 ${global.conns.length} Sub-Bot si sono connessi con successo.`));
        } else {
...
(async () => {
    ...
    try {
        ...
        console.log(chalk.bold.blue(`⭑⭒━━━✦❘༻☾⋆⁺₊✧ KryptaBOT connesso correttamente ✧₊⁺⋆☽༺❘✦━━━⭒⭑`));
...
_quickTest().then(() => conn.logger.info(chalk.bold.blueBright(``)));
